...
def main() -> int:
    ...
    lines += [
        "",
        f"INSERT INTO audit_log (id, actor, action, entity, at, detail) VALUES "
        f"({q('seed:' + now + ':' + uuid.uuid4().hex[:8])}, {q('pipeline')}, {q('seed')}, {q('database')}, {q(now)}, "
        f"{q(json.dumps({'households': len(hh.get('households', [])), 'attenders': len(att.get('attenders', [])), 'candidates': len(ch.get('candidates', [])), 'centroids': len(cent.get('centroids') or {})}))});",
        "COMMIT;",
    ]
