...
| `data/` | Anonymous seed JSON: `{id, lat, lon, zip, flags}` with coordinates rounded to 3 decimals (~110 m). Plus `zip_centroids.csv`, which is published Census/USPS geography. | Yes |
...
data/        anonymous seed JSON + public ZIP reference table
